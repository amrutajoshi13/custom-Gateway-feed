...
proto_464xlat_setup() {
	...
	#above line is commented because 464xlat configuration shld be taken from network file
	...
	echo "$ip6addr" > /tmp/464-$cfg-anycast
         if [ $cfg = "wan6c1_xlat" ] 
         then
             metric=$(uci get mwan3.wan6c1_failover.metric) 
         elif [ $cfg = "wan6c2_xlat" ] 
         then 
             metric=$(uci get mwan3.wan6c2_failover.metric)
         fi
