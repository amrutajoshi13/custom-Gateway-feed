...
then
			...
			echo -e "\e[1;32m [$time_stamp]   Modem/USB test=FAIL \e[0m"
...
do
	...
	rtc_time=$(hwclock -r | cut -d " " -f 1,2,3,4 | cut -d ":" -f 1,2)
...
echo "rtc_time=$rtc_time" > "$outputfile2path"
...
while true; do
    ...
    read -p "Whether all the LED's (Status and Ethernet LED's) are glowing or not? (y/n) " res14
	
    sleep 2
    res14=$(echo "$res14" | tr -d '\011\012\013\014\015\040')
    echo $res14
    time_stamp=$(date)
