L.ui.view.extend({
	...
	title: L.tr('VLAN'),
	description: L.tr('<b>Do not mix tagged and untagged frames on the same physical interface. Please click on update after editing or deleting any changes. Also create an Interface for the created VLAN in Setting-->Network.</b>'),
	...
	pbRenderContents: function (rv) {
		...
		for (var key in rv) {
			if (rv.hasOwnProperty(key)) {
				...
				list.row([VLANID, Port0, Port1, Port4, key]);
			}
		}
	...
	pbSectionAdd: function () {
		...
		///// Condition to check only untagged and untagged///////////////////
		...
		if (status == "Add") {
			...
			if ((key_2 == pbvlan || !id) && !port0 && !port1 && !port2 && !port3 && !port4) {
				...
				if(port0 || port1 || port2 || port3 || port4)
				{
					alert("Do not mix untagged and untagged,tagged and untagged frames on the same physical interface");
				}
				else{
					alert("VLAN ID already exits")
