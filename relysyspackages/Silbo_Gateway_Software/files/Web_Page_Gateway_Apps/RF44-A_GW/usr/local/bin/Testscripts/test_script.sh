...
if [ "$result_logged" = false ]; then
    a=$(echo "[$time_stamp]          $register_message" | tee -a /usr/local/bin/Testscripts/Testresult/$filename.txt)
    result_logged=true
fi 
...
sleep 3
echo -e "\e[1;32m board_powering_off1.Press Enter after 10 seconds to continue the test... \e[0m"
sleep 3
