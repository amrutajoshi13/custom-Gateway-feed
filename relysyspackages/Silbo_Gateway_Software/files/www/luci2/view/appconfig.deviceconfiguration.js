L.ui.view.extend({
    ...
    RS485ConfigSectionAdd: function () {
        ...
        if (sdevicename != "") {
            ...
            let value;
            if (protocol === "TCP") {
                value = $('#stdModbusEnable1').val();
            } else {
                value = $('#stdModbusEnable').val();
            }
    ...
    RS485ConfigSectionEdit: function (ev) {
                    ...
                    if (["primary"].includes(server) && cloudprotocol === 'mqtt') {
                        ...
                        if (data1.topicname) {
                            if (data1.dataSendingMethod === "1" && data1.topicname) {
                                console.log("Entered this 123245logice")
                                ...
                                dropdown1.value = 3;
                                setDisplay("block", "none");
                            } else if (data1.dataSendingMethod === "2") {
                                console.log("Entered this 123logice")
                                dropdown1.value = 2;
                                setDisplay("none", "none");
                            }
                        }
        ...
        $('#serialslaveid').on('input', function () {
            if ($(this).val() !== '') { // If field is not empty, show message
                    ...
                    if (["primary"].includes(server) && cloudprotocol === 'mqtt') {
                        ...
                        if (data2.pseudoTopicName) {
                            if (data2.pseudoDataSendingMethod === "1" && data2.pseudoTopicName) {
                                ...
                                setDisplay("block", "none");
                            } else if (data2.pseudoDataSendingMethod === "2") {
                ...
                _edit_register.innerHTML = `
                    <div class="row" style="margin-top:5px;">
                        ...
                        <div class="col-md-2">
                            ...
                            <input type="number" id="startregister${self.CounterRegister}" class="form-control" name="Start Register">
                        </div>
                        <div class="col-md-2">
                            <label id="requiredfield">Register Count</label>
                            <input type="number" id="registercount${self.CounterRegister}" class="form-control" name="Register Count">
                        </div>
                        <div class="col-md-2">
    ...
    Pseudoeditrows: function (editdata) {
        ...
        function populateRegistersForSelectedSlave(selectedSlaveID, selectedSlaveRegister, modbusDevices, registerDropdownID) {
            ...
            console.log("Dropdown before clearing:", $registerNameDropdown.html());

            let uniqueRegisters = new Set(); // Store unique registers
            let tempRegisterList = []; // Temporary storage for register values
            ...
            self.RS485GetUCISections("registerconfig", "registerconfig").then(function (rv) {
                ...
                if (getregistrdata && Object.keys(getregistrdata).length > 0) {
                    ...
                    console.log("Selected Device ID:", selectedDeviceID, "Selected Slave ID:", selectedSlaveIDOnly);

                    var selectedDevice = Object.values(modbusDevices).find(dev =>
                        (dev.serialdeviceid === selectedDeviceID) && (dev.serialslaveid === selectedSlaveIDOnly)
                    );
                        ...
                        Object.keys(getregistrdata).forEach(function (key) {
                            ...
                            if (sectionName === selectedDeviceID) {
                                ...
                                for (let i = 0; ; i++) {
                                    ...
                                    let registerValue = device[registerKey].trim();

                                    console.log("Checking register:", registerValue);

                                    // Add only unique values
                                    if (!uniqueRegisters.has(registerValue)) {
                                        uniqueRegisters.add(registerValue);
                                        tempRegisterList.push(registerValue);
                                        hasRegisters = true;
                                    } else {
                                        console.log("Skipping duplicate register:", registerValue);
                                    }
                                }
                            }
                        });
                    ...
                    // **Check if dropdown already contains values**
                    let existingOptions = new Set();
                    $registerNameDropdown.find('option').each(function () {
                        existingOptions.add($(this).val());
                    });

                    // **Append only new values**
                    tempRegisterList.forEach(registerValue => {
                        if (!existingOptions.has(registerValue)) {
                            console.log("Appending register to dropdown:", registerValue);
                            let registerOption = new Option(registerValue, registerValue);
                            if (registerValue === selectedSlaveRegister) {
                                registerOption.selected = true;
                            }
                            $registerNameDropdown.append(registerOption);
                        } else {
                            console.log("Register already exists in dropdown, skipping:", registerValue);
                        }
                    });

                    if (!hasRegisters) {
                        console.log("No registers found for:", selectedDeviceID);
                        $registerNameDropdown.empty().append(new Option("No registers found", ""));
                    }
                } else {
                    console.log("No register data available");
                    $registerNameDropdown.empty().append(new Option("No registers found", ""));
                }

                console.log("Dropdown after processing:", $registerNameDropdown.html()); // Log final dropdown content
            })
            // .catch(function (error) {
            //     console.error("Error fetching register data:", error);
            //     $registerNameDropdown.empty().append(new Option("Error loading registers", ""));
            // });
        }
    ...
    AddRegisterSection: function () {
       ...
       <div class="row" style="margin-top:5px";>
            ...
            <div class="col-md-2">
             <label id="requiredfield">Start Register</label>
                <input type="number" id="startregister${self.CounterRegister}" class="form-control inputbox"  name="Start Register">
            </div>
            <div class="col-md-2">
             <label id="requiredfield">Register Count</label>
                <input type="number" id="registercount${self.CounterRegister}" class="form-control inputbox"  name="Register Count">
            </div>
            <div class="col-md-2">
             <label id="requiredfield">Datatype</label>
